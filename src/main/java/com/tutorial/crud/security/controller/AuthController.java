...
public class AuthController {
    ...
    @Autowired
    UsuarioServiceImplementacion userServiceImplement;
    ...
    public ResponseEntity<?> nuevo(@Valid @RequestBody NuevoUsuario nuevoUsuario, BindingResult bindingResult) {
        ...
        if (usuarioService.existsByNombreUsuario(nuevoUsuario.getNombreUsuario()))
            return new z|ResponseEntity(new Mensaje("ese nombre ya existe"), HttpStatus.BAD_REQUEST);
        ...
        if(usuario.getRoles().size() > 1){
            user =
                    new User(true,"admin",nuevoUsuario.getNombreUsuario(), nuevoUsuario.getNombre(),
                            nuevoUsuario.getApellido(),nuevoUsuario.getEmail());
        }else {
            user =
                    new User(true,"user",nuevoUsuario.getNombreUsuario(), nuevoUsuario.getNombre(),
                            nuevoUsuario.getApellido(),nuevoUsuario.getEmail());
        }

        usuarioService.save(usuario);
        userServiceImplement.crearUsuario(user);
